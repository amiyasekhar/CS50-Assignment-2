...
class Sentence():
    ...
    def mark_safe(self, cell):
        ...
        if cell in self.cells:
            self.cells.remove(cell)
    def print(self):
        safe = self.known_safes()
        mine = self.known_mines()
        if safe != set():
            print(self.cells, "SAFES")
            print()
        elif mine != set():
            print(self.cells, "MINES")
            print()
        else:
            print(self.cells, "NOT KNOWN")
            print()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        unident_neighbours = self.find_all_neighbours(cell)
        for neighbour in unident_neighbours:
            if neighbour in self.mines or neighbour in self.safes:
                known_neighbours.add(neighbour)
                if neighbour in self.mines:
                    count -= 1
        
        unident_neighbours = unident_neighbours - known_neighbours
        new_sentence = Sentence(unident_neighbours, count)
        ...
        for sentence in self.knowledge:
            if sentence.cells == set():
                blank_sents.append(sentence)
        for sentence in blank_sents:
        ...
        for sentence in self.knowledge:
            if new_sentence.cells and sentence.cells != new_sentence.cells:
                if sentence.cells.issubset(new_sentence.cells):
                    updated_cells = new_sentence.cells - sentence.cells
                    updated_count = new_sentence.count - sentence.count
                    knowledge_to_add.append(Sentence(updated_cells, updated_count))
